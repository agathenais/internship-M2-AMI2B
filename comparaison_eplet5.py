    ...
    for row in range(len(df[patient_type])):
        ...
        for allele in df.columns[1:]:
            ...
            if allele_list[0] in cell:
                try:
                    eplet_list.extend([x for x in df_1.loc[[cell]].values.flatten().tolist() if str(x) != 'nan'])
                except:
                    KeyError
                    pass
            elif allele_list[1] in cell:
                try:
                    eplet_list.extend([x for x in df_2.loc[[cell]].values.flatten().tolist() if str(x) != 'nan'])
                except:
                    KeyError
                    pass
            elif allele_list[2] in cell:
                try:
                    eplet_list.extend([x for x in df_3.loc[[cell]].values.flatten().tolist() if str(x) != 'nan'])
                except:
                    KeyError
                    pass
...
def row_wise_D_R_comparison(recipientdictunique, donordictunique):
    ...
    for rkey, dkey in zip(Rkeys_list,DKeys_list):
        charge_epitopique = set(donordictunique.get(dkey)).difference(set(recipientdictunique.get(rkey)))
...
def final_eplet_machin_to_csv(verified, donordict_final, recipientdict_final,d3,verified3):
    if verified is not None:
        bestcol = ["Donor", "Donor epitope", "Recipient", "Recipient epitope", "Donor vs recipient", "Epitopic charge", "Donor vs recipient", "Epitopic charge verified"]
    else:
        bestcol = ["Donor", "Donor epitope", "Recipient", "Recipient epitope", "Donor vs recipient", "Epitopic charge"]
    horizontal_concat = pd.concat([donordict_final, recipientdict_final, d3, verified3], axis=1)
    horizontal_concat.columns = bestcol
    horizontal_concat.to_csv(output)
    
def verified_column_only(verifiedonly, donordict_final, recipientdict_final, verified3):
    if verifiedonly is not None:
        bestcol = ["Donor", "Donor epitope", "Recipient", "Recipient epitope", "Donor vs recipient", "Epitopic charge verified"]
        horizontal_concat = pd.concat([donordict_final, recipientdict_final, verified3], axis=1)
        horizontal_concat.columns = bestcol
        horizontal_concat.to_csv(output)
...
if __name__ == '__main__':
    ...
    DP = pd.read_csv(os.path.join(inputdirectory,"DP.csv")).set_index("allele")
    DQ = pd.read_csv(os.path.join(inputdirectory,"DQ.csv")).set_index("allele")
    DR = pd.read_csv(os.path.join(inputdirectory,"DR.csv")).set_index("allele")
    ...
    if classI is not None and classII is None:
        ...
        recipientdictunique = allele_df_to_eplets_dict(recipientdf,'ID_P',1,A,B,C)

    if classII is not None and classI is None:
        donordictunique = allele_df_to_eplets_dict(donordf,'ID_T',2,DP,DQ,DR)
        recipientdictunique = allele_df_to_eplets_dict(recipientdf,'ID_P',2,DP,DQ,DR)

    if classI is not None and classII is not None:
        donordictunique={}
        recipientdictunique={}
        donordict = allele_df_to_eplets_dict(donordf,'ID_T',1, A,B,C)
        recipientdict = allele_df_to_eplets_dict(recipientdf,'ID_P',1,A,B,C)
        donordict_2 = allele_df_to_eplets_dict(donordf,'ID_T',2,DP,DQ,DR)
        recipientdict_2 = allele_df_to_eplets_dict(recipientdf,'ID_P',2,DP,DQ,DR)
    ...
    if verifiedonly is not None:
        ...
        verified_column_only(verifiedonly, donordict_final, recipientdict_final, verified3)
    else: 
        donordict_final = donordict_to_dataframe(donordictunique)
        recipientdict_final = recipientdict_to_dataframe(recipientdictunique)
        d3 = row_wise_D_R_comparison(recipientdictunique, donordictunique)
        verified3 = eplet_cross_verification(d3)
        final_eplet_machin_to_csv(verified, donordict_final, recipientdict_final, d3, verified3)
